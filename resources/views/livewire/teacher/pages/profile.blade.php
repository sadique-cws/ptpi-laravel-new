<div class="p-1 sm:p-6 lg:p-2">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="absolute bottom-0 left-8 transform translate-y-1/2">
                <div class="relative group">
                    ...
                    @elseif (Auth::user()->image)
                        <!-- Current profile image from ImageKit -->
                        <img src="{{ Auth::user()->image }}" alt="Profile Picture"
                            class="w-32 h-32 rounded-full border-4 border-white object-cover shadow-lg transition duration-300">
                    @else
                        <!-- Initials if no image -->
                        <div
                            class="w-32 h-32 rounded-full border-4 border-white bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center shadow-lg">
                            <span
                                class="text-4xl text-gray-400 font-medium">{{ substr($first_name ?? '', 0, 1) }}</span>
                        </div>
                    @endif
                    ...
                    @error('image')
                        <span class="absolute top-full left-0 text-red-500 text-sm mt-2">{{ $message }}</span>
                    @enderror
                </div>
            </div>
        </div>
        ...
        <!-- Success Message -->
        @if (session('success'))
            <div class="absolute top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg">
                {{ session('success') }}
            </div>
        @endif
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center space-x-2 p-3 rounded-lg bg-gray-50">
                            ...
                            <span
                                class="ml-2 px-2 py-1 text-xs font-medium text-teal-700 bg-teal-100 rounded-full">Verified</span>
                        </div>
                    </div>
                    ...
                    @foreach (['first_name' => 'First Name', 'last_name' => 'Last Name', 'phone' => 'Phone Number'] as $field => $label)
                        <div class="space-y-2">
                            ...
                            @else
                                <div
                                    class="flex items-center justify-between p-3 rounded-lg bg-gray-50 group hover:bg-gray-100 transition-all duration-200">
                        ...
                        <div class="space-y-2">
                            ...
                            @else
                                <div
                                    class="flex items-center justify-between p-3 rounded-lg bg-gray-50 group hover:bg-gray-100 transition-all duration-200">
                                    <span
                                        class="text-gray-700">{{ $$field ? ucfirst($$field) : 'Not Provided' }}</span>
                                    <button wire:click="editField('{{ $field }}')"
                                        class="p-1.5 rounded-md text-gray-400 opacity-0 group-hover:opacity-100 hover:text-teal-600 hover:bg-white transition-all duration-200">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
